...
void patchCtrlRaceBaseCount(RaceScreen *raceScreen, u32 controlCount)
{
   ...
   raceScreen->InitControlGroup(controlCount + localPlayerCount + 1);
   return;
}
...
void CtrlRaceDebug::OnUpdate(){
   ...
   if(isPanelVisible){
      ...
      wchar_t debugPanel[0x180];
      const wchar_t *format = L"Respawn %2d\nCurCP    %3d\nCurKCP   %2d\nRace%%    %01.3f\nITPT      %3d\nKCL %02x %02x\n%ls\n%ls\nMP %d";
      u32 length = swprintf(debugPanel, 0x180, format, respawn, cp, kcp, raceComp, ITPT, kclFlag, kclVariant, trick, drop, currentMusicPack);
